...
def find_header_row(raw_rows, max_scan=10):
    """Sheets often have a title row (and blank spacer rows) above the real
    header row - e.g. a merged 'PORTFOLIO' title, then an empty row, then
    the actual column headers. Scan the first few rows and treat the first
    one with at least 2 non-empty cells as the header. Falls back to row 0
    if nothing qualifies (e.g. a single-column table)."""
...
def parse_document(input_path):
    ...
    for sheet in doc.sheets:
        ...
        for table in sheet.tables:
            ...
            if header_idx > 0:
                log(f'"{key}": header found at row {header_idx + 1} (skipped {header_idx} title/spacer row(s))')
